...
public class WordCountSlidingWindow {
    ...
    private void createWCAppAndRun() {
        ...
        KStream<String, String> textLines = builder.stream("wcisw");
...
//        wordCounts.toStream().to("wcotw", Produced.with(windowedSerde, Serdes.Long()));
        wordCounts.toStream()
                .map((Windowed<String> key, Long count) -> new KeyValue<>(key.toString() + "     " +
                        ...
                        count.toString()))
                .to("wcosw", Produced.with(Serdes.String(), Serdes.String()));
